...
class Device:
    ...
    def swap(self, w1: str, w2: str):
        ...
        if w1 in self.constants and w2 in self.constants:
            self.constants[w1], self.constants[w2] = self.constants[w2], self.constants[w1]
        ...
        match self.find_segment():
            case (start, end):
                ...
                for i in range(start, end):
                    ...
                    for w in self.constants.keys():

                        if is_xy(w) or w in self.usage[i]:
                            continue

                        if w not in self.wires:
                            self.getvalue(w)

                        if self.getvalue(w) == 0:
                            zeros.add(w)
                        else:
                            ones.add(w)
