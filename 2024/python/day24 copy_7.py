...
class Device:
    ...
    def find_segment(self, zt: int) -> int | None:
            ...
            if w not in self.swapped:
                # if w not in self.usage[start-1]:
                    ...
                    if not cl.is_recursive(zw, w, set()):
                        print('Try swapping', zw, 'for', w, end='')
                        cl.run(zt)
    ...
    def solve_swap(self, start, zw, zt):
        ...
        if exp == 0:
            ...
            self.try_candidates(self.zeroes, zw, zt)
            ...
            self.try_candidates(self.ones, zw, zt)
            ...
            if start is not None:
                ...
                if is_xy(i1) and is_xy(i2):
                    ...
                    if i1 in self.constants:
                        self.solve_swap(start, i2, zt)
                    else:
                        self.solve_swap(start, i1, zt)
