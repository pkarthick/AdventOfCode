...
def dirpad_keystrokes(s: str, e:  str) -> str:
    ...
    return list(filter(lambda s: len(s) == l, strokes))
...
@cache
def dirpad_keystrokes_by_level1(s, e, level, base) -> list[str]:

    if ((s,e), level) in dir_cache:
        return dir_cache[((s,e), level)]

    else:
        pairs = dirpad_keystrokes_by_level1(s, e, level-base, base)

        s = []

        for (s1, e1) in pairwise(pairs):
            s.extend(dir_cache[((s1,e1), base)])
            s.append('A')
...
def next_level(level):
    ...
    for (s,e) in dirmove.keys():
        ...
        for ks in dir_cache[((s,e), level-1)]:

            strokes = dir_cache[(('A',ks[0]), 1)][:]

            for (s1, e1) in pairwise(ks):
                strokes.extend(dir_cache[((s1,e1), 1)])

            if ((s,e), level) in dir_cache:
                if len(strokes) < len(dir_cache[((s,e), level)]):
    ...
    for st in num_strokes:
        ...
        for (s,e) in pairwise(['A']+st):
            if s == e:
                ...
                tot += len(dirpad_keystrokes_by_level(s, e, 2, 1))
                # tot += 1
