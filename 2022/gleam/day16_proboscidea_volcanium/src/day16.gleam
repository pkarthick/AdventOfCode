...
Valve JJ has flow rate=21; tunnel leads to valve II"
  ...
  compute_max_pressure_released(input)
...
Valve FQ has flow rate=12; tunnels lead to valves QN, WT, UG, RQ, QM"
  ...
  compute_max_pressure_released(input1)
}

fn compute_max_pressure_released(input: String) {
  let data = data.parse(input)

  let path = path.new(data)
  let max_pressure_paths = map.from_list([])
  let max_pressure = 0

  let #(max_pressure, _) =
    path.compute_max_pressure_released(path, max_pressure_paths, max_pressure)

  io.println("Max pressure released: " <> int.to_string(max_pressure))
}
