...
proc findTarget(paths: seq[string]): seq[(int, int)] =
    ...
    for path in paths:
        ...
        of 'U': 
            ...
            y += dist
        of 'D': 
            for d in countdown(dist, 1):
                result.add((x, y-d))
            y -= dist
        else: discard


let set1 = findTarget(input1.split({','})).toHashSet
let set2 = findTarget(input2.split({','})).toHashSet

var common = set1.intersection(set2).toSeq

echo common.mapIt(abs(it[0])+abs(it[1])).sorted()[0]
