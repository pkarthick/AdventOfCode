...
proc confirmsRule(r: int, msg: string) : (bool, string) =
    
    let subrules = rules[r]
    var submsg = msg
    ...
    for subrule in subrules:
        ...
        for r in subrule:
            ...
            if rules[r].len == 0: #Not a rule
                if submsg.startsWith(parts[r]):
                    submsg = submsg[parts[r].len .. ^1]
                    continue
                else:
                    match = false
                    break
            else: #A rule
                let (ok, sb) = confirmsRule(r, submsg)
                if ok:
                    submsg = sb
                    continue
                else:
