...
let input="""light salmon bags contain 5 dotted olive bags, 4 wavy lavender bags.
...
proc newBag(line: string): Bag =
    ...
    for l in innerlist:
        let bagsegs = l.split(" ").filterIt(not it.isEmptyOrWhitespace)
...
let bags = input.split('\n').filterIt(not it.isEmptyOrWhitespace).mapIt(newBag(it))
let bagsTable = bags.mapIt((it.color, it)).toSeq.toTable
let colorToFind = "shiny gold"

proc contains(bag: Bag, color: string): bool =
    
    if bag.inner.len == 0:
        false
    elif bag.inner.anyIt(it.color == color):
        true
    else:
        bag.inner.anyIt(bagsTable[it.color].contains(color))

echo bags.filterIt(it.color != colorToFind and it.inner.len > 0 and contains(it, colorToFind)).len