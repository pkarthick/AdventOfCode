...
# let input1="""ecl:gry pid:860033327 eyr:2020 hcl:#fffffd# byr:1937 iyr:2017 cid:147 hgt:183cm
...
type 
    Passport = object
        ...
        validations: Table[string, Validation] 

proc getValidation(key: string): Validation = 
    case key 
    of "ecl": 
        newContainsValidation(@["amb", "blu", "brn", "gry", "grn", "hzl", "oth"])
    of "hcl": 
        newGroupValidation(
            @[ 
                newLengthValidation(7), 
                newHasItemValidation((0, '#')), 
                newInCharRangesValidation(@['0'..'9', 'a'..'f'], proc(val: string): string = val[1..^1]) 
            ])
    of "hgt": 
        newOrValidation(
            newGroupValidation(
                @[ 
                        newEndsWithValidation("cm"), 
                        newIntegerValidation(proc(val: string): string = val[0..^3]), 
                        newInRangeValidation(150 .. 193, proc(val: string): string = val[0..^3]) 
                    ]),
                newGroupValidation(
                    @[ 
                            newEndsWithValidation("in"), 
                            newIntegerValidation(proc(val: string): string = val[0..^3]), 
                            newInRangeValidation(59 .. 76, proc(val: string): string = val[0..^3]) 
                        ])
            )
    of "byr": 
        newGroupValidation(
            @[
                newLengthValidation(4), 
                newIntegerValidation(), 
                newInRangeValidation(1920 .. 2002) 
                ]) 
    of "iyr": 
        newGroupValidation(
            @[
                newLengthValidation(4), 
                newIntegerValidation(), 
                newInRangeValidation(2010 .. 2020) 
                ]) 
    of "eyr": 
        newGroupValidation(
            @[
                newLengthValidation(4), 
                newIntegerValidation(), 
                newInRangeValidation(2020 .. 2030) 
                ]) 
    of "pid": 
        newGroupValidation(
            @[ 
                newLengthValidation(9), 
                newIntegerValidation() 
                ]) 
    else: nil #TODO: error handling

proc newPassport(info: string): Passport = 
    result = Passport(
        table: initTable[string, string](), 
        validations: initTable[string, Validation](),
        requiredKeys: @["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]
    )
    
    for key in result.requiredKeys:
        result.validations[key] = getValidation(key)    

    for line in info.splitLines: 
        for seg in line.split: 
            let kv = seg.split({':'}) 
            result.table[kv[0]] = kv[1]

proc hasKey(passport: Passport, key: string): bool =
    passport.table.hasKey(key)

proc checkValidity(passport: Passport): bool =
    
    for key, validation in passport.validations.pairs: 
        if not passport.hasKey(key): 
            echo "key not present: " , key
            return false
        else: 
            let val = passport.table[key]
            if not validation.validate(val): 
                echo key, ":", val, " is not valid" #done 
                return false
