...
def number_of_arrangements(s: str, group_sizes: List[int], gi: int, start: int, count: int) -> int:
    ...
    if check_groups(s_hash, group_sizes, gi+1):
        ...
        if start+group_sizes[gi-1]+1 <= len(pattern): 
            if s[start+group_sizes[gi-1]+1] == '?':
                s_dot = s.replace("?", ".", 1)
                
                dot_count = number_of_arrangements(s_dot, group_sizes, gi+1, start+group_sizes[gi-1]+1, count+1)
                if dot_count != -1:
                    return dot_count
        ...
        if check_groups(s_dot, group_sizes, gi+1):
            dot_count = number_of_arrangements(s_dot, group_sizes, gi, start+1, 0)
            if dot_count != -1:
                return dot_count            
...
for lin in lines:
    ...
    num = number_of_arrangements(pattern, sizes, 0, 1, 0)
    count += num
